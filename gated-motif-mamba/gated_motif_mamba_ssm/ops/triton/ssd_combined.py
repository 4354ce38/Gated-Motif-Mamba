...
from gated_motif_mamba_ssm.utils.torch import custom_bwd, custom_fwd
...
from gated_motif_mamba_ssm.ops.triton.ssd_bmm import _bmm_chunk_fwd, _bmm_chunk_bwd
from gated_motif_mamba_ssm.ops.triton.ssd_chunk_state import _chunk_cumsum_fwd, _chunk_cumsum_bwd
from gated_motif_mamba_ssm.ops.triton.ssd_chunk_state import _chunk_state_fwd, _chunk_state_bwd_db
from gated_motif_mamba_ssm.ops.triton.ssd_chunk_state import _chunk_state_bwd_ddAcs_stable
from gated_motif_mamba_ssm.ops.triton.ssd_chunk_state import chunk_state, chunk_state_ref
from gated_motif_mamba_ssm.ops.triton.ssd_chunk_state import chunk_state_varlen
from gated_motif_mamba_ssm.ops.triton.ssd_state_passing import _state_passing_fwd, _state_passing_bwd
from gated_motif_mamba_ssm.ops.triton.ssd_state_passing import state_passing, state_passing_ref
from gated_motif_mamba_ssm.ops.triton.ssd_chunk_scan import _chunk_scan_fwd, _chunk_scan_bwd_dz, _chunk_scan_bwd_dstates
from gated_motif_mamba_ssm.ops.triton.ssd_chunk_scan import _chunk_scan_bwd_dC, _chunk_scan_bwd_dcb
from gated_motif_mamba_ssm.ops.triton.ssd_chunk_scan import _chunk_scan_bwd_ddAcs_stable
from gated_motif_mamba_ssm.ops.triton.ssd_chunk_scan import chunk_scan, chunk_scan_ref
from gated_motif_mamba_ssm.ops.triton.ssd_chunk_scan import _chunk_scan_bwd_ddAcs_prev
from gated_motif_mamba_ssm.ops.triton.layernorm_gated import rmsnorm_fn, _layer_norm_fwd, _layer_norm_bwd
from gated_motif_mamba_ssm.ops.triton.k_activations import _swiglu_fwd, _swiglu_bwd
from gated_motif_mamba_ssm.utils.determinism import (
...
def mamba_chunk_scan(
    ...
    Argument:
        ...
        dt_bias: (nheads,)
        initial_states: (batch, nheads, headdim, dstate)
        input_gate: (batch, nheads, nchunks)
        pq_P: (dim, rank) or (nheads, dim, rank)
        pq_Q: (rank, dim) or (nheads, rank, dim)
    Return:
        out: (batch, seqlen, nheads, headdim)
        chunk_states: (batch, nchunks, nheads, headdim, dstate), if return_chunk_states=True
        final_states: (batch, nheads, headdim, dstate), if return_final_states=True
    """
    batch, seqlen, nheads, headdim = x.shape
    dstate = B.shape[-1]
    if initial_states is not None:
    ...
    # Use the total elapsed dt in each chunk to scale the PQ interaction once per chunk.
    pq_dt = dt.sum(dim=-1) if (pq_P is not None or pq_Q is not None) else None
    ...
    if pad_len > 0:
        out = out[:, :seqlen]
    if return_chunk_states and return_final_states:
        return out, states, final_states
...
def ssd_selective_scan(x, dt, A, B, C, D=None, z=None, dt_bias=None, dt_softplus=False, dt_limit=(0.0, float("inf"))):
    ...
    Return:
        out: (batch, seqlen, nheads, headdim)
    """
    from gated_motif_mamba_ssm.ops.selective_scan_interface import selective_scan_fn
