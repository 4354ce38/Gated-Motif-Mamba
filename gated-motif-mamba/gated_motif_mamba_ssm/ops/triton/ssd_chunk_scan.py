...
def chunk_scan(B, C, x, dt, dA_cumsum, prev_states, D=None, z=None):
    ...
    device_map = {name: tensor.device for name, tensor in tensors.items() if tensor is not None}
    non_cuda = {name: str(device) for name, device in device_map.items() if device.type != "cuda"}
    if non_cuda:
        raise RuntimeError(
            "chunk_scan received non-CUDA tensors before Triton launch: "
            f"{non_cuda}; all devices={device_map}"
        )
